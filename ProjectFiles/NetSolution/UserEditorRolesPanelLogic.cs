...
public class UserEditorRolesPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIRoles()
    {
        ...
        foreach (var role in roles.Children)
        {
            ...
            if (editable.Value)
            {
                roleUiObject = InformationModel.MakeObject<Panel>(role.BrowseName, Demo_Features_Pier.ObjectTypes.GroupCheckbox);
            }
            else if (UserHasRole(role.NodeId))
            {
                roleUiObject = InformationModel.MakeObject<Panel>(role.BrowseName, Demo_Features_Pier.ObjectTypes.GroupLabel);
            }
