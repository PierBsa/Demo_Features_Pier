...
public class UserEditorGroupsPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            ...
            if (editable.Value)
            {
                groupUiObject = InformationModel.MakeObject<Panel>(group.BrowseName, Demo_Features_Pier.ObjectTypes.GroupCheckbox);
            }
            else if (UserHasGroup(group.NodeId))
            {
                groupUiObject = InformationModel.MakeObject<Panel>(group.BrowseName, Demo_Features_Pier.ObjectTypes.GroupLabel);
            }
